...
// 转换 MDX 动画数据为 Three.js AnimationClip - 使用新的封装函数
function convertAnimationsToThreeJS(sequences, bones) {
    console.log('=== 转换 MDX 动画到 Three.js (使用封装函数) ===');
    ...
    // 使用新的封装函数进行转换
    const clips = window.convertAnimationsToThreeJS ?
        window.convertAnimationsToThreeJS(sequences, bones, {
            fps: 30,
            logDetails: true,
            validateData: true
        }) :
        // 如果新的封装函数不可用，使用原有的实现
        convertAnimationsToThreeJSLegacy(sequences, bones);
...
// 原有的实现作为后备
function convertAnimationsToThreeJSLegacy(sequences, bones) {
    console.log('=== 转换 MDX 动画到 Three.js (原有实现) ===');
